...
/*   Updated: 2023/12/28 21:23:27 by dnikifor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long	getting_new_colour(t_wf *frame, float incr, long a, long b)
{
	float	incr_sum[3];
	long	new_col;
	...
	frame->incr[2] = (float)(((b >> 8) & 0xFF) - ((a >> 8) & 0xFF))
		/frame->max;
	incr_sum[0] = ((a >> 24) & 0xFF) + frame->incr[0] * incr;
	incr_sum[1] = ((a >> 16) & 0xFF) + frame->incr[1] * incr;
	incr_sum[2] = ((a >> 8) & 0xFF) + frame->incr[2] * incr;
...
long	set_x(t_wf *frame, t_map *matrix, float incr)
{
	...
	new_col = getting_new_colour(frame, incr, a, b);
	return (new_col);
}
